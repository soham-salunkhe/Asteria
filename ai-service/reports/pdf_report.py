...
def generate_pdf(run_id: str) -> bytes:
    ...
    # Measured tracking-phase pixel stats from sampled telemetry
    # (TRACKING/LOCKED-gated, same population the live metrics use).
    # No degree→pixel conversion estimates — only real measurements.
    import math as _math
    _px = [s.get('pixel_error_total') for s in samples
           if s.get('target_state') in ('TRACKING', 'LOCKED')
           and s.get('pixel_error_total') is not None]
    _n_px = len(_px)
    _avg_px = sum(_px) / _n_px if _n_px else None
    _max_px = max(_px) if _px else None
    _rmse_px = (_math.sqrt(sum(v * v for v in _px) / _n_px)
                if _n_px >= 2 else None)
    ...
    # Header
    content.append(Paragraph('ASTERIA', title_style))
    content.append(Paragraph('AI-Based Virtual Camera Tracking — FSOC Coarse Alignment · PS169', subtitle_style))
    ...
    _px_str = lambda v, u: f"{v:.2f} {u}" if v is not None else '— (no tracking samples)'
    content.append(kv_table([
        ('Duration',               f"{dur:.2f} s"),
    ...
    content.append(Spacer(1, 6))
    content.append(section('3. PS169 Spec Compliance'))
    ...
    # Measured pixel errors only — never a degree→pixel estimate.
    spec_data = [
        ...
        spec_row('Avg Tracking Error', '≤ 10 px',
                 f"{_avg_px:.1f} px" if _avg_px is not None else '— (no tracking samples)',
    ...
    content.append(Paragraph(
        f'Generated by ASTERIA — {_ts(time.time())}',
